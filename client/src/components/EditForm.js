...
const EditForm = () => {
    ...
    const { id } = useParams()
    useEffect(() => {
        axios.get(`http://localhost:8001/api/findById/${id}`).then(
            (res) => {
                dispatch({ type: "SET_YEAR", payload: res.data.year })
                dispatch({ type: "SET_MANUFACTURER", payload: res.data.manufacturer })
                dispatch({ type: "SET_CATEGORY", payload: res.data.category })
                dispatch({ type: "SET_DESCRIPTION", payload: res.data.description })
                dispatch({ type: "SET_MODEL_NUMBER", payload: res.data.modelNumber })
                dispatch({ type: "SET_SERIAL_NUMBER", payload: res.data.serialNumber })
                dispatch({ type: "SET_QUANTITY", payload: res.data.quantity })
                dispatch({ type: "SET_VOLTAGE", payload: res.data.voltage })
                dispatch({ type: "SET_OWNER", payload: res.data.owner })
    ...
    return (
        <div className='py-10'>
            <h1 className='text-4xl font-thin'>Update a new Machine</h1>
            <form onSubmit={submitHandler} className='flex flex-col p-2' enctype='multipart/form-data'>
                <label>Year</label>
                <input className='border border-gray-400' type="number" name='year' onChange={(e) => handleChange(e)} value={state.year} />
